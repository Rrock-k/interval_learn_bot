...
| `AWAITING_GRADE_TIMEOUT_MS` | нет | Через сколько миллисекунд ожидания убрать кнопки и вернуть карточку в очередь (по умолчанию 300000, т. е. 5 минут) |
| `AWAITING_GRADE_RETRY_MINUTES` | нет | Через сколько минут после таймаута повторно напоминать карточку (по умолчанию 60) |
