...
test('sendReminderJobToChannel отправляет reminder через reply к существующей базе', async () => {
  const card = createCard({ baseChannelMessageId: 777 });
...
test('sendReminderJobToChannel без базы копирует исходник и ставит кнопки на него', async () => {
  const card = createCard({ baseChannelMessageId: null });
  const job = createJob(card);
  const store = createStore({ cardById: card });
  const telegram = createTelegram({
    copyMessage: async () => ({ message_id: 555 }),
  });
  const scheduler = new ReviewScheduler(store.mock as any, telegram.mock);
  ...
  assert.equal(telegram.calls.copyMessage[0]?.[2], card.sourceMessageId);
  assert.ok(telegram.calls.copyMessage[0]?.[3]?.reply_markup);
...
test('sendReminderJobToChannel fallback на сохранённый текст при невалидном исходнике', async () => {
