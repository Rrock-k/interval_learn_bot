...
const TECHNICAL_PREVIEWS = new Set(['[Фото]', '[Видео]']);

const isTechnicalPreview = (value: string | null) => {
  if (value === null) return true;
  return (
    TECHNICAL_PREVIEWS.has(value) ||
    value.startsWith('[Фото x') ||
    value.startsWith('[Видео x') ||
    value.startsWith('[Медиа x')
  );
};

const usage = () => `
Usage:
  npm run recover:photo-captions -- [options]

Modes:
  default       List DB candidates only. Does not call Telegram.
  --probe      Forward source messages to read caption/text. Does not update DB.
  --apply      Forward source messages and update DB when real caption/text is found.
...
const listCandidates = async (pool: Pool, options: Options) => {
  ...
  const where = [
    `content_type IN ('photo', 'video')`,
    `(
      content_preview IS NULL
      OR content_preview IN ('[Фото]', '[Видео]')
      OR content_preview LIKE '[Фото x%'
      OR content_preview LIKE '[Видео x%'
      OR content_preview LIKE '[Медиа x%'
    )`,
  ];
...
const forwardAndRecover = async ({
  ...
  return {
    ...
    recoveredPreview:
      recoveredPreview && !isTechnicalPreview(recoveredPreview)
        ? recoveredPreview
        : null,
    error: null,
  };
  ...
  const { rows } = await pool.query(
    `
      ...
      SET content_preview = $1,
          ...
          updated_at = $2
      WHERE id = $3
      RETURNING id, content_preview, base_channel_message_id
    `,
